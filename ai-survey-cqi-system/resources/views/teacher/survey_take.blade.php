...
<div class="container py-5">
    <div class="row justify-content-center">
        <div class="col-lg-10 col-xl-8">
            ...
            {{-- Survey Form --}}
            <form id="survey-form" action="{{ route('teacher.surveys.submit', $survey->id) }}" method="POST">
                @csrf
                                            ...
                                            <div class="d-flex gap-3">
                                                @for ($i = 1; $i <= 5; $i++)
                                                    <div class="form-check form-check-inline">
                                                        ...
                                                        <label class="form-check-label fw-bold text-dark" for="q{{ $question->id }}_{{ $i }}">{{ $i }}</label>
                                                    </div>
                                                @endfor
                                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // beforeunload handler to warn leaving without submitting
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Remove beforeunload to prevent "changes may not be saved"
        window.removeEventListener('beforeunload', beforeUnloadHandler);
