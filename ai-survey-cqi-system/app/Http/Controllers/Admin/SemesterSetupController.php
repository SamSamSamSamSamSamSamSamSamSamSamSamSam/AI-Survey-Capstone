...
{
    // Steps definition — used by the progress tracker
    public const STEPS = [
    ...
    // -------------------------------------------------------------------------
    // Main wizard view — shows current step
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // STEP 1: Register Students via CSV
    // -------------------------------------------------------------------------
        ...
        DB::transaction(function () use ($rows, $studentRole, &$created, &$skipped, &$errors) {
            foreach ($rows as $i => $row) {
                ...
                $user = User::create([
                    ...
                    'password'          => Hash::make(trim($row['user_id_number'])), // Default: ID number
    ...
    // -------------------------------------------------------------------------
    // STEP 3: Import Course Offerings via CSV
    // -------------------------------------------------------------------------
    ...
    public function importOfferings(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($rows, $activeSemester, &$created, &$skipped, &$errors) {
            foreach ($rows as $i => $row) {
                ...
                if (! $subject) {
                    $errors[] = "Row {$line}: Subject '{$row['subject_code']}' not found.";
                    continue;
                }

                $teacher = User::where('user_id_number', trim($row['teacher_id_number']))->first();
                if (! $teacher) {
                    $errors[] = "Row {$line}: Teacher '{$row['teacher_id_number']}' not found.";
                    continue;
                }
                ...
                if (! empty($row['block_name'])) {
                    $block = Block::where('name', strtoupper(trim($row['block_name'])))
                                  ...
                                  ->first();
                    if (! $block) {
                        $errors[] = "Row {$line}: Block '{$row['block_name']}' not found for active semester.";
                        continue;
                    }
                    $blockId = $block->id;
                }

                // Optional offering type
                $offeringTypeId = null;
                if (! empty($row['offering_type'])) {
                    $ot = OfferingType::where('name', 'like', trim($row['offering_type']))->first();
                    $offeringTypeId = $ot?->id;
                }

                $groupNumber = ! empty($row['group_number']) ? (int) $row['group_number'] : null;

                // Check uniqueness: group_number + semester_id + subject_id
    ...
    // -------------------------------------------------------------------------
    // STEP 4: Import Enrollments via CSV
    // -------------------------------------------------------------------------
    ...
    public function importEnrollments(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($rows, $activeSemester, $defaultType, &$created, &$skipped, &$errors) {
            foreach ($rows as $i => $row) {
                ...
                if (! $student) {
                    $errors[] = "Row {$line}: Student '{$row['student_id_number']}' not found.";
                    continue;
                }
    ...
    private function parseCsv(\Illuminate\Http\UploadedFile $file): array
    {
        ...
        while (($data = fgetcsv($handle)) !== false) {
            if ($headers === null) {
                ...
                $headers = array_map(fn ($h) => strtolower(trim(str_replace("\xEF\xBB\xBF", '', $h))), $data);
                continue;
            }
